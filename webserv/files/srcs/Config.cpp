...
void Config::_parseServerBlock(const std::string& block, ServerConfig& config) {
	...
	for (size_t i = 0; i < lines.size(); ++i)
	{
		...
		//
		if (seen_ports.count(lines[i].port))
		{
			std::cerr << "Error: duplicate listen port " << lines[i].port << std::endl;
			throw std::runtime_error("Duplicate port in config");
		}
		seen_ports.insert(lines[i].port);
		//
