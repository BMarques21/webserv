...
void Config::_parseConfigFile(const std::string& path){
	...
	// Find all server blocks — skip matches inside other keywords like "server_name"
	size_t pos = 0;
	while ((pos = content.find("server", pos)) != std::string::npos) {
		// Make sure this is a standalone "server" keyword, not "server_name" etc.
		size_t after = pos + 6; // length of "server"
		if (after < content.size() && content[after] != ' ' && content[after] != '\t'
		    && content[after] != '\n' && content[after] != '\r' && content[after] != '{') {
			pos = after;
			continue;
		}
		// Also skip if preceded by a non-whitespace char (e.g. inside a comment line)
		if (pos > 0 && content[pos - 1] != '\n' && content[pos - 1] != '\r'
		    && content[pos - 1] != ' ' && content[pos - 1] != '\t') {
			pos = after;
			continue;
		}
		// Skip comment lines: find start of this line
		size_t line_start = content.rfind('\n', pos);
		line_start = (line_start == std::string::npos) ? 0 : line_start + 1;
		std::string before_keyword = content.substr(line_start, pos - line_start);
		// trim
		size_t first_non_ws = before_keyword.find_first_not_of(" \t");
		if (first_non_ws != std::string::npos && before_keyword[first_non_ws] == '#') {
			pos = after;
			continue;
		}
