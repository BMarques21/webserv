...
bool UploadHandler::parseMultipartFormData(const std::string& body,
    ...
    while (pos < body.length()) {
        ...
        std::string content;
        if (content_end > pos) {
            content = body.substr(pos, content_end - pos);
        }
        // else: empty content (0-length file), content stays empty
...
HttpResponse UploadHandler::handleUpload(const HttpRequest& request) {
    ...
    // Empty body is a valid POST - return 200 OK
    if (request.getBody().empty()) {
        return HttpResponse::ok("OK");
    }

    // If not multipart/form-data, acknowledge plain POST and return early
    ...
    std::vector<UploadedFile> files;
    parseMultipartFormData(request.getBody(), boundary, files);

    // No files to save - still a valid request
    if (files.empty()) {
        return HttpResponse::ok("<html><body><h1>200 OK</h1><p>No files uploaded</p></body></html>", "text/html");
    }
